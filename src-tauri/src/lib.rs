use repoatlas_core::core::activity::{
...
use repoatlas_core::{
    ...
    scan::ScanEngine,
    ActivityHistoryResponse, AppSettings, AtlasReport, AttentionCenterState, Broker, Core,
    DashboardSnapshot, ExternalTools, GitDiff, GitOp, GitStatus, LogChunk, PendingApproval,
    PortConflict, ProjectCollection, ProjectPatch, ProjectQuery, ProjectRemoval, ProjectSummary,
    ScanProgress, ScanResult, ScanRoot, ScanRootRemoval, SearchHit, TaskDefinition, TaskRun,
    TaskRuntimeRequest, TaskSpec,
};
use serde::Serialize;
...
async fn get_activity_history(
    state: State<'_, Arc<AppState>>,
    query: ActivityQuery,
) -> Result<ActivityHistoryResponse, String> {
...
async fn get_activity_summary(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        core.lock()
            .map_err(|e| e.to_string())?
            .activity_summary(&days, project_id.as_deref())
            .map_err(err_to_string)
    })
    .await
    .map_err(|e| e.to_string())?
...
async fn get_activity_detail(
    state: State<'_, Arc<AppState>>,
    id: String,
) -> Result<repoatlas_core::ActivityHistoryItem, String> {
    let core = state.core.clone();
    tauri::async_runtime::spawn_blocking(move || {
        core.lock()
            .map_err(|e| e.to_string())?
...
#[tauri::command]
fn start_git_history_refresh(
    state: State<Arc<AppState>>,
    request: HistoryRefreshRequest,
) -> Result<HistoryRefreshStatus, String> {
    state
        .history
        .start(state.core.clone(), request)
        .map_err(err_to_string)
}
#[tauri::command]
fn get_git_history_refresh_status(state: State<Arc<AppState>>) -> HistoryRefreshStatus {
    state.history.status()
}
#[tauri::command]
fn cancel_git_history_refresh(state: State<Arc<AppState>>, id: String) {
    state.history.cancel(&id);
}
