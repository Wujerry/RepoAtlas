...
import type { ToastTone, UpdateState } from "../types";
...
export function UpdateSettings({ state, locale, t, notify, onCheck, onDownload, onInstall, onRestart, onDefer }: UpdateSettingsProps) {
  ...
  const checkedAt = formatUpdateDate(state.checkedAt, locale);
  ...
  return (
    <section className="settings-card settings-updater" aria-busy={isBusy}>
      ...
      {state.status === "error" && <motion.div key="error" className="settings-update-status is-error" role="alert" {...fadeMotion}><WarningCircle className="settings-update-icon" aria-hidden="true" /><div className="settings-update-copy"><strong>{updateErrorTitle(state.errorStage, t)}</strong><span>{state.error || t("unknownUpdateError")}</span></div><Button variant="quiet" disabled={isBusy} loading={busy === "check" || busy === "download" || busy === "install" || busy === "restart"} onClick={() => void retry()}>{t("retry")}</Button></motion.div>}
      </AnimatePresence>
