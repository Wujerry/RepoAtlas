...
function Set-RepoAtlasVersion {
  ...
  $insecureProperty = $config.plugins.updater.PSObject.Properties["dangerousInsecureTransportProtocol"]
  if ($null -eq $insecureProperty) {
...
function Get-NsisInstaller {
  ...
  if (-not $installer) {
    throw "No NSIS setup executable for version $Version found below $bundleDir"
  }
  ...
  foreach ($root in $roots) {
    $entry = Get-ItemProperty -Path $root -ErrorAction SilentlyContinue |
      Where-Object {
        ...
        $property -and [string]$property.Value -eq "RepoAtlas"
      } |
      Select-Object -First 1
...
function Wait-ForInstallVersion {
  ...
  do {
    ...
    if ($entry) {
      $displayVersion = Get-PropertyValue -Object $entry -Name "DisplayVersion"
      if ([string]$displayVersion -eq $Version) {
        return $entry
      }
    }
  ...
  $currentVersion = if ($current) { [string](Get-PropertyValue -Object $current -Name "DisplayVersion") } else { "<not installed>" }
...
function Get-MainExecutable {
  ...
  $location = [string](Get-PropertyValue -Object $InstallEntry -Name "InstallLocation")
  $location = $location.Trim('"')

  if ([string]::IsNullOrWhiteSpace($location)) {
    $uninstallString = Get-PropertyValue -Object $InstallEntry -Name "UninstallString"
    if ($uninstallString) {
      $uninstallPath = ([string]$uninstallString).Trim().Trim('"')
...
function Uninstall-RepoAtlas {
  $entry = Get-RepoAtlasInstallEntry
  if (-not $entry) {
    return
  }
  $uninstallString = Get-PropertyValue -Object $entry -Name "UninstallString"
  if (-not $uninstallString) {
    return
  }
  $uninstall = ([string]$uninstallString).Trim().Trim('"')
