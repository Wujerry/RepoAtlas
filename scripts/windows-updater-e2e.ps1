  ...
  foreach ($root in $roots) {
    $entry = Get-ItemProperty -Path $root -ErrorAction SilentlyContinue |
      Where-Object {
        ...
        $displayName -and [string]$displayName.Value -eq "RepoAtlas"
      } |
      Select-Object -First 1
...
function Get-MainExecutable {
  ...
  $uninstallProperty = $InstallEntry.PSObject.Properties["UninstallString"]
  if ([string]::IsNullOrWhiteSpace($location) -and $uninstallProperty -and $uninstallProperty.Value) {
    $uninstallPath = ([string]$uninstallProperty.Value).Trim().Trim('"')
...
try {
  ...
  if ($existing -and $existing.PSObject.Properties["UninstallString"] -and $existing.PSObject.Properties["UninstallString"].Value) {
    $uninstall = ([string]$existing.PSObject.Properties["UninstallString"].Value).Trim().Trim('"')
  ...
  $entry = Get-RepoAtlasInstallEntry
  if ($entry -and $entry.PSObject.Properties["UninstallString"] -and $entry.PSObject.Properties["UninstallString"].Value) {
    $uninstall = ([string]$entry.PSObject.Properties["UninstallString"].Value).Trim().Trim('"')
    if (Test-Path -LiteralPath $uninstall) {
